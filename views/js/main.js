...
function updatePositions() {
  ...
  for (var i = 0; i < items.length; i++) {
      ...
      var leftPos = items[i].basicLeft + 100 * phase;
      if((0 - items[i].width) <= leftPos && leftPos <= window.innerWidth) {
//          only display image in viewport, the rest of the image is set to display none.
        items[i].style.left = leftPos + 'px';
          items[i].style.display = '';
      } else {
//          display none only hide the element, but it should not effect the layout
//          visibile property does effect the layout. see http://www.w3schools.com/css/css_display_visibility.asp
